...
class periodized_stokes_fmm(object):
    ...
    def __internal_call__(self, source, reflection_distance, forces, dipstr, dipvec, target, compute_source_velocity, compute_source_stress, compute_target_velocity, compute_target_stress, loc):
        ...
        if loc == 'lb':
            flux_check_x = self.check[:,self.slice_right].copy()
            flux_check_x[0] -= 0.25*self.ranx
            flux_check_y = self.check[:,self.slice_top].copy()
            flux_check_y[1] -= 0.25*self.rany
        elif loc == 'lt':
            flux_check_x = self.check[:,self.slice_right].copy()
            flux_check_x[0] -= 0.25*self.ranx
            flux_check_y = self.check[:,self.slice_bottom].copy()
            flux_check_y[1] += 0.25*self.rany
        elif loc == 'rb':
            flux_check_x = self.check[:,self.slice_left].copy()
            flux_check_x[0] += 0.25*self.ranx
            flux_check_y = self.check[:,self.slice_top].copy()
            flux_check_y[1] -= 0.25*self.rany
        elif loc == 'rt':
            ...
            raise Exception('loc ' + str(loc) + 'not recognized')
        def sum_check(u, sl):
        ...
        out3 = SFMM(
                    source = all_source,
                    target = flux_check_x,
                    forces = all_forces if forces is not None else None,
                    dipstr = all_dipstr if dipstr is not None else None,
                    dipvec = all_dipvec if dipvec is not None else None,
                    compute_target_velocity = True,
                )
        out4 = SFMM(
                    source = self.source,
        ...
        flux_x = out3['target']['u'] + out4['target']['u']
        out3 = SFMM(
                    source = all_source,
                    target = flux_check_y,
                    forces = all_forces if forces is not None else None,
        ...
        flux_y = out3['target']['v'] + out4['target']['v']
        if loc == 'lb':
            sum_flux_x = sum_check(flux_x, self.slice_right)
            sum_flux_y = sum_check(flux_y, self.slice_top)
        elif loc == 'lt':
            sum_flux_x = sum_check(flux_x, self.slice_right)
            sum_flux_y = sum_check(flux_y, self.slice_bottom)
        elif loc == 'rb':
            sum_flux_x = sum_check(flux_x, self.slice_left)
            sum_flux_y = sum_check(flux_y, self.slice_top)
        elif loc == 'rt':
            sum_flux_x = sum_check(flux_x, self.slice_left)
            sum_flux_y = sum_check(flux_y, self.slice_bottom)
        ...
        if compute_source_velocity or compute_source_stress:
            ...
            if compute_source_velocity:
                out_self['u'] = out1['source']['u'][:SN] + out2['target']['u'][:SN] - sum_flux_x
                out_self['v'] = out1['source']['v'][:SN] + out2['target']['v'][:SN] - sum_flux_y
        ...
        if compute_target_velocity or compute_target_stress:
            ...
            if compute_target_velocity:
                out_target['u'] = out1['target']['u'] + out2['target']['u'][SN:] - sum_flux_x
                out_target['v'] = out1['target']['v'] + out2['target']['v'][SN:] - sum_flux_y
    ...
    def __call__(self, source, reflection_distance=0.15, forces=None, dipstr=None, dipvec=None, target=None, compute_source_velocity=False, compute_source_stress=False, compute_target_velocity=False, compute_target_stress=False):
        ...
        out_lb = self.__internal_call__(source, reflection_distance, lb_forces, lb_dipstr, lb_dipvec, target, compute_source_velocity, compute_source_stress, compute_target_velocity, compute_target_stress, 'lb')
        ...
        out_lt = self.__internal_call__(source, reflection_distance, lt_forces, lt_dipstr, lt_dipvec, target, compute_source_velocity, compute_source_stress, compute_target_velocity, compute_target_stress, 'lt')
        ...
        out_rb = self.__internal_call__(source, reflection_distance, rb_forces, rb_dipstr, rb_dipvec, target, compute_source_velocity, compute_source_stress, compute_target_velocity, compute_target_stress, 'rb')
        ...
        out_rt = self.__internal_call__(source, reflection_distance, rt_forces, rt_dipstr, rt_dipvec, target, compute_source_velocity, compute_source_stress, compute_target_velocity, compute_target_stress, 'rt')

        out = {}
        if compute_source_velocity or compute_source_stress:
            out_self = {}
            if compute_source_velocity:
                out_self['u'] = out_lb['self']['u'] + out_lt['self']['u'] + out_rb['self']['u'] + out_rt['self']['u']
                out_self['v'] = out_lb['self']['v'] + out_lt['self']['v'] + out_rb['self']['v'] + out_rt['self']['v']
            if compute_source_stress:
                out_self['u_x'] = out_lb['self']['u_x'] + out_lt['self']['u_x'] + out_rb['self']['u_x'] + out_rt['self']['u_x']
                out_self['u_y'] = out_lb['self']['u_y'] + out_lt['self']['u_y'] + out_rb['self']['u_y'] + out_rt['self']['u_y']
                out_self['v_x'] = out_lb['self']['v_x'] + out_lt['self']['v_x'] + out_rb['self']['v_x'] + out_rt['self']['v_x']
                out_self['v_y'] = out_lb['self']['v_y'] + out_lt['self']['v_y'] + out_rb['self']['v_y'] + out_rt['self']['v_y']
                out_self['p']   = out_lb['self']['p']   + out_lt['self']['p']   + out_rb['self']['p']   + out_rt['self']['p']
            out['self'] = out_self
        if compute_target_velocity or compute_target_stress:
            out_target = {}
            if compute_target_velocity:
                out_target['u'] = out_lb['target']['u'] + out_lt['target']['u'] + out_rb['target']['u'] + out_rt['target']['u']
                out_target['v'] = out_lb['target']['v'] + out_lt['target']['v'] + out_rb['target']['v'] + out_rt['target']['v']
            if compute_target_stress:
                out_target['u_x'] = out_lb['target']['u_x'] + out_lt['target']['u_x'] + out_rb['target']['u_x'] + out_rt['target']['u_x']
                out_target['u_y'] = out_lb['target']['u_y'] + out_lt['target']['u_y'] + out_rb['target']['u_y'] + out_rt['target']['u_y']
                out_target['v_x'] = out_lb['target']['v_x'] + out_lt['target']['v_x'] + out_rb['target']['v_x'] + out_rt['target']['v_x']
                out_target['v_y'] = out_lb['target']['v_y'] + out_lt['target']['v_y'] + out_rb['target']['v_y'] + out_rt['target']['v_y']
                out_target['p']   = out_lb['target']['p']   + out_lt['target']['p']   + out_rb['target']['p']   + out_rt['target']['p']
            out['target'] = out_target
        return out
